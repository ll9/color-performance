...
function startup() {
  ...
  for (let index = 0; index < 500; index++) {
    let rawHtml = `
        <div class="sp-replacer sp-light" id="${`color-picker-${index}`}" onclick=toggleColorPicker(this)>
        ...
        `;
    parent.appendChild(htmlToElement(rawHtml));
  }
  console.timeEnd("startup");
}

function toggleColorPicker(element) {
  const tooltip = document.getElementById("color-popover-container");
